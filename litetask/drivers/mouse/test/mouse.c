...
extern int initSerial(char *device, int speed);
extern int readSerial(int, char *, int);
extern int writeSerial(int, char *, int);
...
/* Make sure we are running at old speed */
	if(!old)
		old = 1200;
	initSerial("COM1", old);

/* Now write to mouse to tell it to change baud rate */
  if(!new)
	new = 1200;
  switch(new) {
  case 9600:  c = "*q"; break;
  ...
  writeSerial(x386Info.mseFd, c, 2);  
  for(i=0; i<30000; i++);
	  ...
	  int        what;
{
  ...
  switch (what)
	 ...
	 case DEVICE_ON:
		if ((x386Info.mseFd = open(x386Info.mseDevice, O_RDWR | O_BINARY)) < 0)
	{
		...
		if (x386Info.mseType != P_BM) 
	{
	  ...
	  if (x386Info.mseType == P_LOGI)
		 {
			writeSerial(x386Info.mseFd, "S", 1);
			x386SetSpeed(x386Info.baudRate, x386Info.baudRate, cflag[P_MM]);
		 }

	  if      (x386Info.sampleRate <=   0)  writeSerial(x386Info.mseFd, "O", 1);
	  else if (x386Info.sampleRate <=  15)  writeSerial(x386Info.mseFd, "J", 1);
	  else if (x386Info.sampleRate <=  27)  writeSerial(x386Info.mseFd, "K", 1);
	  else if (x386Info.sampleRate <=  42)  writeSerial(x386Info.mseFd, "L", 1);
	  else if (x386Info.sampleRate <=  60)  writeSerial(x386Info.mseFd, "R", 1);
	  else if (x386Info.sampleRate <=  85)  writeSerial(x386Info.mseFd, "M", 1);
	  else if (x386Info.sampleRate <= 125)  writeSerial(x386Info.mseFd, "Q", 1);
	  else                                  writeSerial(x386Info.mseFd, "N", 1);
	}
	 ...
	 case DEVICE_CLOSE:
		if (x386Info.mseType == P_LOGI)
	{
	  writeSerial(x386Info.mseFd, "U", 1);
...
x386MseEvents()
{
  ...
  if (!(nBytes = readSerial(x386Info.mseFd, (char *)rBuf, sizeof(rBuf)))) return;
...
main (int argc, char **argv)
{
  ...
  if (argc != 3) {
	 fprintf (stderr, "Usage: mouse [Microsoft|MouseSystem|MMseries|Logitech] device\n");
	 exit (1);
  }
