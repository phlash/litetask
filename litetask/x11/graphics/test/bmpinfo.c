...
/* Dump info */
   printf("File signature: %.2s\n", &info.bf.bfType);
   printf("File size:      %ld\n", info.bf.bfSize);
   printf("Offset to bits: %ld\n", info.bf.bfOffsetBits);
   putchar('\n');
   printf("Bitmap size:    %ld x %ld\n", info.bi.biWidth, info.bi.biHeight);
   printf("Bitmap planes:  %d\n", info.bi.biPlanes);
   printf("Bitmap bits/pix:%d\n", info.bi.biBitsPerPixel);
   printf("Bitmap compress:%ld\n", info.bi.biCompression);
...
/* set video mode */
   ...
   case 8:
      ...
      break;
   default:
      printf("Unsupported number of colors :)\n");
      goto Done;
   }
